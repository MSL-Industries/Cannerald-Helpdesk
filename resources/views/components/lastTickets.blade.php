@if($ticket->requester->tickets()->latest()->where('id', '!=', $ticket->id)->count())
    ...
    Last 5 Tickets:
        @foreach($ticket->requester->tickets()->latest()->where('id', '!=', $ticket->id)->limit(5)->get() as $otherTicket)
            <div>
                @busy <span class="label ticket-status-{{ $otherTicket->statusName() }}">{{ __("ticket." . $otherTicket->statusName() ) }}</span> &nbsp;
                @busy <span class="label ticket-priority-{{ $otherTicket->priorityName() }}">{{ __("ticket." . $otherTicket->priorityName() ) }}</span> &nbsp;
                @busy <span class="label" style="background-color:{{$otherTicket->type->color ?? "white"}}"> {{ $otherTicket->type->name ?? "--" }}</span>
